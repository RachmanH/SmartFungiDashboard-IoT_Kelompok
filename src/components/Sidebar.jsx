    ...
    <>
      ...
      <aside
        className={`
          ...
          bg-white dark:bg-neutral-800 border-r-4 border-black dark:border-white
      ...
      >
        {/* Logo section */}
        <div className={`
          h-14 min-h-14 border-b-4 border-black dark:border-white
        ...
        `}>
          <div className="flex items-center gap-2 overflow-hidden transition-all duration-300">
            ...
            <span className={`font-black text-lg text-black dark:text-white tracking-tight whitespace-nowrap transition-all duration-200 ${labelClass}`}>
              SmartFungi
            </span>
          </div>
        </div>
            ...
            {navItems.map(({ to, label, icon: Icon }) => {
              ...
              return (
                <Link
                  ...
                  onClick={onCloseMobile}
                  className={`
                    flex items-center gap-3 py-3 font-bold border-2 border-black dark:border-white rounded-xl
                    transition-all duration-150 active:scale-95
                    ${expandedClass}
                    ${active
                      ? 'bg-yellow-300 dark:bg-yellow-400 text-black shadow-neo-sm'
                      : 'bg-white dark:bg-neutral-900 text-black dark:text-white hover:bg-cream-200 dark:hover:bg-neutral-700'
                    }
                  `}
        ...
        {/* Footer */}
        <div className={`
          py-4 border-t-4 border-black dark:border-white
          ${mobileOpen ? 'px-4 md:px-2' : desktopOpen ? 'px-4' : 'px-2'}
          transition-all duration-300
        `}>
          <div className={`
            text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center
